...
class InvoiceGenerationService {
    ...
    for (const item of lineItems) {
      if (!item.unitPrice || item.unitPrice === 0) {
        try {
          ...
          console.warn(`Failed to apply pricing for item ${item.ndisItemNumber}:`, error.message);
  ...
  /**
   ...
   async getNdisStandardPricing(ndisItemNumber, state = 'NSW') {
     try {
       ...
       console.error(`Error getting NDIS standard pricing for ${ndisItemNumber}:`, error);
       return null;
     }
   }
  ...
  async generateInvoiceLineItems(userEmail, clientEmail, startDate, endDate) {
    try {
      ...
      console.error('Error generating invoice line items:', error);
      throw error;
    }
  }
  ...
  async createLineItemFromSchedule(scheduleItem, assignment, client, workedTimeData) {
    try {
      ...
      if (!ndisItem || !ndisItem.itemNumber) {
        console.warn(`No NDIS item found for schedule on ${scheduleItem.date}`);
        return null;
      }
      ...
      if (!pricing || pricing.price === null || pricing.price === undefined || isNaN(pricing.price)) {
        console.warn(`Invalid pricing for NDIS item ${ndisItem.itemNumber}:`, pricing);
        return null;
      }
      ...
      console.error('Error creating line item from schedule:', error);
      return null;
    }
  }
  ...
  async createLineItemFromWorkedTime(workEntry, assignment, client) {
    try {
      ...
      if (!ndisItem) {
        console.warn(`NDIS item ${ndisItemNumber} not found`);
        return null;
      }
      ...
      // Validate pricing before calculations
      if (!pricing || pricing.price === null || pricing.price === undefined || isNaN(pricing.price)) {
        console.warn(`Invalid pricing for NDIS item ${ndisItemNumber}:`, pricing);
      ...
      console.error('Error creating line item from worked time:', error);
      return null;
    }
  }
  ...
  async getApprovedExpensesForInvoice(organizationId, clientId, startDate, endDate) {
    try {
      console.log('🔍 DEBUG: getApprovedExpensesForInvoice called with:');
      ...
      console.log('🔍 DEBUG: Using clientId for query:', clientIdForQuery, 'type:', typeof clientIdForQuery);
      ...
      console.log('🔍 DEBUG: Query clientId string:', clientIdForQuery);
      ...
      console.log('🔍 DEBUG: Test query without clientId found:', testExpenses.length, 'expenses');
      
      if (testExpenses.length > 0) {
        console.log('🔍 DEBUG: Sample expense clientId type:', typeof testExpenses[0].clientId, 'value:', testExpenses[0].clientId);
      }
      ...
      if (expenses.length > 0) {
        console.log('💰 DEBUG: Sample expense:', JSON.stringify(expenses[0], null, 2));
      }
      ...
      console.error('Error retrieving approved expenses for invoice:', error);
  ...
  async convertExpenseToLineItem(expense) {
    try {
      ...
      console.error('Error converting expense to line item:', error);
      return null;
    }
  }
  ...
  async getPricingForItem(ndisItemNumber, organizationId, clientId, state = 'NSW', providerType = 'standard') {
    try {
      ...
      console.error('Error getting pricing for item:', error);
      return {
        price: 0,
        source: 'error',
        isCustom: false,
  ...
  async getNdisItemDetails(itemNumber) {
    try {
      ...
      console.error('Error fetching NDIS item details:', error);
      return null;
    }
  }
    ...
    try {
      ...
      if (!skipPriceValidation && lineItems.length > 0) {
        try {
          ...
          console.warn('Price validation failed:', priceValidationError.message);
  ...
  async validateLineItemPricing(lineItems, defaultState = 'NSW', defaultProviderType = 'standard') {
    try {
      ...
      console.error('Error in line item pricing validation:', error);
