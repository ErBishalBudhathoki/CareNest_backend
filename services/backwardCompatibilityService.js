...
class BackwardCompatibilityService {
  ...
  async transformLegacyInvoice(legacyInvoice) {
    try {
      ...
      logger.error('Error transforming legacy invoice', {
        error: error.message,
        stack: error.stack,
        legacyInvoiceId: legacyInvoice._id,
        organizationId: legacyInvoice.organizationId
      });
  ...
  async mapLegacyItemToNdis(legacyItem) {
    try {
      ...
      logger.error('Error mapping legacy item to NDIS', {
        error: error.message,
        stack: error.stack,
        legacyItem
      });
  ...
  async processLegacyInvoice(legacyInvoiceData) {
    try {
      ...
      logger.error('Error processing legacy invoice', {
        error: error.message,
        stack: error.stack
      });
      return {
  ...
  async migrateLegacyInvoices(options = {}) {
    try {
      ...
      logger.info('Legacy invoices migration started', {
        invoiceCount
      });
      ...
      while (await cursor.hasNext()) {
        ...
        if (migrationStats.totalProcessed % 50 === 0) {
          logger.info('Legacy invoices migration progress', {
      ...
      logger.error('Error during legacy invoice migration', {
        error: error.message,
        stack: error.stack
      });
      throw error;
  ...
  async getLegacyDataStats() {
    try {
      ...
      logger.error('Error getting legacy data stats', {
        error: error.message,
        stack: error.stack
      });
      throw error;
